...
export const ClientBriefs = () => {
  ...
  return (
    <Layout>
      <Navbar />
      <div className="p-6 h-[90vh] overflow-y-auto space-y-8">
        {/* Page Header */}
        <div>
        ...
        <div className="flex gap-3">
          {["newest", "oldest"].map((order) => (
            <button
              ...
              className={`px-5 py-2 rounded-full border text-sm font-medium shadow-sm transition-all duration-300 flex items-center gap-2
                ${
                  sortOrder === order
        ...
        <div className="space-y-8">
          {getSortedBriefsData().map((monthData, index) => (
            ...
            >
              {/* Month Header */}
              <div className="px-6 py-4 border-b border-gray-100 bg-gradient-to-r from-gray-100 to-gray-50 rounded-t-2xl flex items-center gap-2">
                    ...
                    >
                      ...
                      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center p-4 gap-3">
                        ...
                        <div className="flex items-center gap-3 flex-wrap justify-end">
                          <span className="flex items-center gap-1 bg-blue-50 text-blue-700 px-3 py-1 rounded-full font-medium text-xs sm:text-sm">
                            <FiBookOpen className="text-base" /> {period.articles} Articles
                          </span>
                          <span className="text-gray-400">•</span>
                          <span className="flex items-center gap-1 bg-pink-50 text-pink-700 px-3 py-1 rounded-full font-medium text-xs sm:text-sm">
                            <FiMessageSquare className="text-base" /> {period.viralDiscussions} Viral Discussions
                          </span>

                          {/* Action Buttons */}
                          <button
                            onClick={() => toggleSummary(id)}
                            className="px-3 py-1 bg-gradient-to-r from-blue-600 to-blue-500 text-white text-xs sm:text-sm font-medium rounded-lg hover:from-blue-700 hover:to-blue-600 transition-all duration-200 flex items-center gap-1"
                          >
                            <FiEye />
                          ...
                          <Link
                            ...
                            className="px-3 py-1 bg-gradient-to-r from-gray-200 to-gray-100 text-gray-800 text-xs sm:text-sm font-medium rounded-lg hover:from-gray-300 hover:to-gray-200 transition-all duration-200 flex items-center gap-1"
                          >
                            <FiArrowRightCircle /> Go to Discussions
                          </Link>
                        </div>
                      </div>
                      ...
                      <div
                        className={`overflow-hidden transition-all duration-300 ease-in-out ${
                          isOpen ? "max-h-60 p-4 pt-0" : "max-h-0 p-0"
                        }`}
                      >
                        <div className="border-t border-gray-200 pt-4">
                          <h3 className="text-base font-semibold text-gray-900 mb-2">Exalt Summary</h3>
