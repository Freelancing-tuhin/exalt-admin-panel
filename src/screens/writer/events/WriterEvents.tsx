...
export default function WriterEvents() {
  ...
  const [sidebarUploadedFile, setSidebarUploadedFile] = useState<File | null>(
    null
  ); // For the actual file object
  const [sidebarLocalPreviewUrl, setSidebarLocalPreviewUrl] = useState<
    string | null
  >(null); // For temporary client-side preview
  ...
  const handleSidebarMediaUpload = (
  ...
  const handleSidebarMediaUrlChange = (
            ...
            <div className="flex flex-wrap gap-3">
              {[
                ...
                <button
                  ...
                  onClick={() =>
                    toggleTag(
                      setSelectedDiasporaTags,
                      selectedDiasporaTags,
                      tag
                    )
                  }
        ...
        <section className="bg-white p-8 rounded-2xl shadow-lg mb-8 border border-gray-100 relative">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
            {[
              {
                title: "Cultural",
                tags: ["Art", "Dance", "Theatre", "Music"],
              },
              {
                title: "Political",
                tags: ["Dinner", "Charity Event", "Rally", "Summit"],
              },
              {
                title: "Religious",
                tags: ["Service", "Performance", "Festival", "Procession"],
              },
              {
                title: "Other",
                tags: [
                  ...
                  {category.tags.map((tag) => (
                    <button
                      ...
                      className={`px-4 py-2 rounded-full text-sm font-medium transition duration-200 ease-in-out shadow-sm
                          ${
                            (
                              selectedCategoryTags[category.title] || []
                            ).includes(tag)
            ...
            <div className="bg-white p-8 rounded-2xl shadow-lg border border-gray-100">
              ...
              <div className="mt-4 space-y-2">
                ...
                {currentSidebarPreviewSource && (
                  <div className="mt-2 p-3 border border-gray-200 rounded-md  bg-white text-center shadow-sm">
                    {currentSidebarPreviewSource.match(
                      /\.(jpeg|jpg|gif|png|webp)$/i
                    ) ? (
                      <img
                        src={currentSidebarPreviewSource}
                        alt={mediaCaption || "Media preview"}
                        className="max-w-full h-auto mx-auto object-contain max-h-60 rounded-md"
                      />
                    ) : currentSidebarPreviewSource.match(
                        /\.(mp4|webm|ogg)$/i
                      ) ? (
                      <video
                        controls
                        src={currentSidebarPreviewSource}
                        className="max-w-full h-auto mx-auto max-h-60 rounded-md"
                      >
                        Your browser does not support the video tag.
                      </video>
                    ) : (
                      <p className="text-sm text-gray-500">
                        Cannot preview this media type directly. URL:{" "}
                        {currentSidebarPreviewSource}
                      </p>
                    )}
