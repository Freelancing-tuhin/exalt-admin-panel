...
interface ArticleProps {
  article: any; // Accept the full article object
}
...
export const Article = ({ article }: ArticleProps) => {
  ...
  // Separate fields
  const { data, questions_from_the_community } = article;
  const graphImages = data?.graph_img_links || [];
  const referenceImage = data?.image || null;

  // Filter out predefined text fields for context-like display
  const textFieldsToShow = Object.entries(data).filter(
    ([key, value]) =>
      key !== "graph_img_links" && key !== "image" && typeof value === "string"
  );

  return (
    <div className="p-2 rounded-xl font-sans relative bg-gray-100">
      <div className="max-w-5xl mx-auto space-y-4">
        {/* Render text fields like context */}
        {textFieldsToShow.map(([key, value], idx) => (
          <motion.section
            key={key}
            initial={{ opacity: 0, y: 20 }}
            whileInView={{ opacity: 1, y: 0 }}
          ...
          >
            <div className="flex items-center gap-3 mb-6">
              ...
              <h2 className="text-xl font-heading font-semibold text-gray-900 border-b-4 border-blue-800 inline-block pb-2 capitalize">
                {key.replace(/_/g, " ")}
              </h2>
            </div>
            <p className="text-lg leading-relaxed tracking-wide text-gray-800 font-normal">
          ...
          >
            <div className="flex items-center gap-4 mb-8">
              ...
              <div>
                ...
                <p className="text-gray-600 text-lg mt-2 font-medium">
                  Visual graphs related to the article.
                </p>
              </div>
            </div>

            <div className="flex justify-center gap-6 overflow-x-auto custom-scrollbar">
              {graphImages.map((imgSrc: string, idx: number) => (
                <div
                  key={idx}
                  className="flex justify-center items-center margin-auto w-full rounded-2xl overflow-hidden border border-gray-200 shadow-md"
                >
          ...
          >
            ...
            <div className="flex justify-center">
              <img
                src={referenceImage}
                alt="Reference"
                className="w-full max-w-2xl h-auto rounded-2xl shadow-md border border-gray-200"
              />
            </div>
          </motion.section>
        )}
        ...
        {/* Questions from the community */}
        {questions_from_the_community?.length ? (
          <motion.section
            initial={{ opacity: 0, y: 20 }}
            whileInView={{ opacity: 1, y: 0 }}
            transition={{ duration: 0.6 }}
          ...
          >
            ...
            <div className="space-y-6">
              {questions_from_the_community.map((q: any, index: number) => (
                <motion.div
                  key={index}
                  initial={{ opacity: 0, y: 10 }}
                  whileInView={{ opacity: 1, y: 0 }}
